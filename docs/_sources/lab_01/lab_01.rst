...
* une archive :file:`drv-2024-boot.tar.gz` contenant les fichiers nécessaires au boot de la DE1-SoC [disponible `ici <http://reds-data.heig-vd.ch/cours/2024_drv/drv-2024-boot.tar.gz>`__]
* une archive :file:`drv-2024-sdcard.tar.gz` contenant une image d'une carte SD pour la DE1-SoC [disponible `ici <http://reds-data.heig-vd.ch/cours/2024_drv/drv-2024-sdcard.tar.gz>`__]
...
.. code-block:: console
   ...
   HOST:~$ tar xzvf drv-2024-sdcard.tar.gz
...
le fichier du Device Tree (:file:`socfpga.dtb`) contenus dans l'archive :file:`drv-2024-boot.tar.gz`.
...
.. admonition:: **Exercice 2**
    ...
    Ecrivez un script (à l'aide d'une variable d'environnement) U-Boot qui va alterner, chaque seconde, entre allumer les 3 affichages 7-segments à gauche et les 3 à droite.
...
.. admonition:: **Exercice 3**
    ...
    Ecrivez un logiciel C qui commence par afficher 0 sur l'affichage HEX0.
    En suite, l'utilisateur peut contrôller le nombre affiché de la façon suivante :

      - En appuyant sur KEY0, le nombre est incrémenté
      - En appuyant sur KEY1, le nombre est décrémenté

    Le nombre affiché est limité entre 0 et 99. Si les limites sont atteintes, il n'est alors plus possible de décrémenté (0 atteint) ou d'incrémenté (99 atteint).

    Egalement, la LED correspondant à l'unité du nombre affiché doit être allumée. Ex. avec 25, la LED5 est allumée.
